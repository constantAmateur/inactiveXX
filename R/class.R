...
print.inactiveXX = function(x,...){
  ...
  cat(sprintf(" %d cells with %d SNPs\n",length(fit$states),length(fit$genotype)))
  cat(sprintf(" %d cells (%.02f%%) called with high confidence\n",sum(fit$cellSummary$highConfCall),sum(fit$cellSummary$highConfCall)/length(fit$states)*100))
  cat(sprintf(" %d SNPs (%.02f%%) called with high confidence\n",sum(fit$snpSummary$highConfCall),sum(fit$snpSummary$highConfCall)/length(fit$genotype)*100))
